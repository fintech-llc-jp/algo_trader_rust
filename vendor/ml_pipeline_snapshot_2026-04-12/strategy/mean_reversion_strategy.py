...
class MeanReversionStrategy:
    ...
    def add_mean_reversion_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['price_mean'] = df['mid_price'].rolling(window=window, min_periods=30).mean()
        df['price_std'] = df['mid_price'].rolling(window=window, min_periods=30).std()
        
        df['price_zscore'] = np.where(
            df['price_std'] > 1e-6,
            (df['mid_price'] - df['price_mean']) / df['price_std'],
            0
        )
        ...
        # ボラティリティ
        df['volatility'] = df['mid_price'].rolling(window=30).std()
    ...
    def generate_labels(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        """
        # Mean Reversion特徴量を追加（ラベル生成時にも追加）
        ...
        buy_mask = (zscore < -threshold) & (future_return > threshold)
        labels[buy_mask] = 1
        
        # SELLシグナル: Zスコアが正（価格が高い）かつ将来価格下落（平均回帰）
        sell_mask = (zscore > threshold) & (future_return < -threshold)
        labels[sell_mask] = -1
