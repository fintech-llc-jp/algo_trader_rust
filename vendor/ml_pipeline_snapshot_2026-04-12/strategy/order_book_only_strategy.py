"""
Order Book Only Strategy
板情報のみを使用して1分後の約定価格を予測する戦略
"""
import pandas as pd
import numpy as np
from typing import Dict, Tuple
from data.order_book_only_feature_engineering import OrderBookOnlyFeatureEngineer
from models.order_book_only_model import OrderBookOnlyModel
from config.settings import ModelConfig, BacktestConfig


class OrderBookOnlyStrategy:
    """
    板情報のみを使用した価格予測戦略
    ...
    def __init__(self, timeframes: list = [1, 5, 30], 
        ...
        """
        self.model = OrderBookOnlyModel()
        self.feature_engineer = OrderBookOnlyFeatureEngineer()
    ...
    def generate_labels(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # 特徴量を抽出（約定データ関連を除外）
        exclude_cols = ['symbol', 'mid_price', 'exec_price_all', 'exec_price_buy', 'exec_price_sell',
    ...
    def train(self, df: pd.DataFrame) -> Dict:
        """モデルを訓練"""
        # 板情報のみの特徴量エンジニアリング
        df = self.feature_engineer.engineer_features(df.copy())
