...
class VolumePredictionStrategy:
    """
    取引量予測戦略
    複数のタイムフレームの特徴量を組み合わせて将来の取引量を予測
    """
    ...
    def __init__(self, timeframes: list = [1, 5, 30], 
        ...
        """
        self.model = VolumePredictionModel()
    ...
    def add_multi_timeframe_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 取引量関連の特徴量
        if 'exec_qty_all' in df.columns:
            for tf in self.timeframes:
                df[f'volume_ma_{tf}s'] = df['exec_qty_all'].rolling(window=tf).mean()
                df[f'volume_std_{tf}s'] = df['exec_qty_all'].rolling(window=tf).std()
    ...
    def generate_labels(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Returns:
            (X, y)
            y: 将来の取引量（prediction_horizon秒後の取引量）
        """
        df = self.add_multi_timeframe_features(df.copy())
        
        # 将来の取引量を取得
        if 'exec_qty_all' in df.columns:
            future_volume = df['exec_qty_all'].shift(-self.prediction_horizon)
        else:
            # 取引量データがない場合は、価格変動から推定
            price_col = 'exec_price_all' if 'exec_price_all' in df.columns else 'mid_price'
            future_volume = df[price_col].pct_change().abs() * 1000  # 仮の取引量
