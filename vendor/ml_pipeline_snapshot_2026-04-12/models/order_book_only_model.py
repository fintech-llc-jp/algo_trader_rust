"""
Order Book Only Model (Regression)
板情報のみを使用して1分後の約定価格を予測する回帰モデル
"""
import os
import joblib
...
class OrderBookOnlyModel:
    """
    板情報のみを使用した価格予測用の回帰モデル
    XGBoost Regressorを使用して将来の約定価格を予測
    約定データは使用せず、板情報のみを使用
    """
    ...
    @classmethod
    def load(cls, filepath: str) -> 'OrderBookOnlyModel':
        """
        モデルをファイルからロード
        ...
        Returns:
            ロードされたOrderBookOnlyModelインスタンス
        """
